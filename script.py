...
def main():
    ...
    poke_list = get_pokemon(search_term)                #Use the returned Pokemon name to search for abilities
    if poke_list:                                                     #If it is a valid Pokemon
        title, body_text = get_paste_data(poke_list, search_term)      #Add the Pokemon and the abilities to the new paste as the title and body text
...
def get_paste_data(poke_list, search_term):                 #This function nabs the paste data
    ...
    body_text = '- ' + divider(poke_list)                    #The body text will be each ability with the divider in effect
